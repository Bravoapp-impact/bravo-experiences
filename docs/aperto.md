...
**Sweep "no-card" non ancora completata.** Il refactor del 2026-05-15 ha appiattito le pagine più visibili (HR Home/Employees/Experiences, AssociationHome, SuperAdminDashboard, UsersPage e tutte le liste che passano da `CrudTableCard`), ma restano da revisionare con la stessa regola: HR `HRTBRequestDetailPage`/`HRTBProposalDetailPage` + modali HR (`HRBookingsDialog`, `EmployeeParticipationsDialog`, `BookingsTable`, `TopPerformersTable`); Super Admin `TBRequestDetailPage`/`TBFormatDetailPage`/`tb-quote-editor/*` + modali (`ExperienceDateDialog`, `TBFormatEditDialog`, `VisibilityDialog`); Association `AssociationHistoryPage`, `AssociationPublicProfile`, modali condivisi (`BookingDetailModal`, `FeedbackModal`). Eccezioni esplicite (da non toccare): `MetricCard`, sidebar prenotazione su `ExperienceDetail`, card item di griglia (`HRExperienceCard`, `BravoCard`, ecc.).
