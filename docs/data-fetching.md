...
**Devtools TanStack Query: non installati al momento.** Bravo! non ha oggi un ambiente di staging separato dalla produzione. Reintroduciamo `@tanstack/react-query-devtools` quando avremo una separazione ambientale sicura. Fino ad allora, debuggare tramite `devLog` e osservazione del comportamento reale (skeleton che appare o non appare alla navigazione).
